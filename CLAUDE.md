...
- 「公開」の自動確定可否はユーザーに一度確認を取ってから進める(2026-08-16時点では
  ...
  履歴で簡単にロールバックできるため)。
- 「実機動作確認まで行う」ときは、実装した機能のシナリオだけでなく
  `/canvas-app-qa-checklist` も確認すること(画面再訪問時の入力残留、データの
  鮮度/Refresh、同日アクションの順序安定性など、個別シナリオのテストでは
  見落としやすい横断的な観点をまとめてある。2026-09-15にフォーム入力残留バグを
  ユーザー報告で発覚させてしまった反省から作成)。
